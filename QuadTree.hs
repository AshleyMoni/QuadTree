...
printZone :: Show a => QuadZone a -> String
printZone zone = breakString (zoneWidth zone) $ concat string
  where string   = map show charGrid
        charGrid = [getLocation (x,y) zone |
...
putZone :: Show a => QuadZone a -> IO ()
putZone = putStr . printZone
